...
import { lorentzForce } from '../../engine/forces/lorentz';
import { useSimulationStore } from '../../store/useSimulationStore';
import { useUIStore } from '../../store/useUIStore';

const SOFTENING = 0.3;

export function LorentzVisualizer() {
  const forcesRef = useRef<THREE.Vector3[]>([new THREE.Vector3(), new THREE.Vector3()]);

  useSimulationLoop(lorentzForce, SOFTENING);

  useFrame(() => {
    const bodies = useSimulationStore.getState().bodies;
    if (bodies.length >= 2) {
      const forces = lorentzForce(bodies, SOFTENING);
